...
This defines piece-wise constant time step sizes (\textit{dtSteps}) over user-defined intervals (\textit{tSteps}), whether dynamic time-step reduction is active, which output quantity is monitored for refinement, the refinement scaling and lower bound, the start and end time of the analysis, and how often output is written.
